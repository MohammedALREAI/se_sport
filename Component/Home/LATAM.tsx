...
const singleNews = ({ news }) => {
  ...
  return (
    <div className=" full w-full h-40 my-2  bg-gray-100 border-2 mx-4  border-gray-200">
      <img src={news.image} alt={news.alt} className="h-full mr-4 rounded border-2" />
      <div className="">
        <h1 className="h-16 overflow-hidden">{news.title}</h1>
        <p className="h-20 overflow-hidden " style={{ textOverflow: "ellipsis" }} ref={descRef}>
          {news.description}
        </p>
      </div>
    </div>
    ...
    <>
      ...
      <div>
        {allNews.map((item, index) => (
          <singleNews news={item} key={`index${index}`} />))}
      </div>
      ...
      <table>
        <tbody className="table-auto my-4 p-4 border-2 border-gray-200 w-full">
          {ranking.map((item, index) => (
            <tr className={index % 2 == 0 ? "bg-gray-100 hover:bg-gray-200" : ""} key={`${index}${item.name}`}>
              <td className="border-2 px-4 py-2"># {index + 1}</td>
              <td className="border-2 px-4 py-2">{item.name}</td>
              <td className="border-2 px-4 py-2"><img src={item.image} alt={item.name} /></td>
...
const RankingElement = () => {
  ...
  return (
    <>
      ...
      <div className="flex">
        <div className={` cursor-pointer px-4 border-l border-r border-t border-gray-200 ${selectedTab == "CURRENT" ? "bg-gray-200 hover-bg-gray-300" : ""}`} onClick={() => SetSelectedTab("CURRENT")}>
          current Stage</div>
        <div className={` cursor-pointer px-4 border-2 border-gray-200 ${selectedTab == "NEXT" ? "bg-gray-200 hover-bg-gray-300" : ""}`} onClick={() => SetSelectedTab("NEXT")}> next Stage</div>
        <div className={` cursor-pointer px-4 border-2 border-gray-200 ${selectedTab == "OVERALL" ? "bg-gray-200 hover-bg-gray-300" : ""}`} onClick={() => SetSelectedTab("OVERALL")}>overall</div>
      </div>
      <RankingTabs ranking={(() => {
        switch (selectedTab) {
          case "CURRENT": return rankingData[0]
            break;
          case "NEXT": return rankingData[1]
            break;
          case "OVERALL": return rankingData[2]
            break;

          default: return rankingData[0]
            break;
        }
...
export const LATAM = ({ allNews }: { allNews: any[] }) => {
  return (
    <div className="flex flex-wrap-reverse container justify-around container mx-auto">
      <div className=" w-full md:w-1/2 px-8 w-full">
